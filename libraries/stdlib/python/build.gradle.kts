...
description = "Kotlin Standard Library for Python backend"

val unimplementedNativeBuiltIns =
    (file("$rootDir/core/builtins/native/kotlin/").list().toSortedSet() - file("$rootDir/libraries/stdlib/python/builtins/kotlin/").list())
        ...
        doFirst {
            publishing.publications {
                if (singleOrNull()?.name != "Main") {
                    throw GradleException("kotlin-stdlib-python should have only one publication, found $size: ${joinToString { it.name }}")
                }
            }
        }
    }
