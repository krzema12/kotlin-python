...
import org.jetbrains.kotlin.python.test.utils.DirectiveTestUtils
import org.jetbrains.kotlin.python.test.utils.ExceptionThrowingReporter
import org.jetbrains.kotlin.python.test.utils.JsTestUtils
import org.jetbrains.kotlin.python.test.utils.verifyAst
...
//        doTest(filePath, "OK", MainCallParameters.noCall(), coroutinesPackage)
    ...
    open fun doTest(filePath: String, expectedResult: String, mainCallParameters: MainCallParameters) {
        ...
        TestFileFactoryImpl().use { testFactory ->
            ...
            val generatedJsFiles = orderedModules.asReversed().mapNotNull { module ->
                val dependencies = module.dependenciesSymbols.map { modules[it]?.outputFileName(outputDir) + ".meta.py" }
                val allDependencies = module.allTransitiveDependencies().map { modules[it]?.outputFileName(outputDir) + ".meta.py" }
                val friends = module.friendsSymbols.map { modules[it]?.outputFileName(outputDir) + ".meta.py" }

                val outputFileName = module.outputFileName(outputDir) + ".py"
                val dceOutputFileName = module.outputFileName(dceOutputDir) + ".py"
                val pirOutputFileName = module.outputFileName(pirOutputDir) + ".py"
            ...
            val inputJsFiles = inputFiles
                    ...
                    .map { inputJsFile ->
                        ...
                        val targetFile = File(outputDir, inputJsFile.module.outputFileSimpleName() + "-py-" + sourceFile.name)
            ...
            val additionalMainJsFile = filePath.removeSuffix("." + KotlinFileType.EXTENSION) + "__main.py"
            ...
            if (!dontRunGeneratedCode && generateNodeJsRunner && !SKIP_NODE_JS.matcher(fileContent).find()) {
                ...
                val nodeRunnerText = generatePythonRunner(allJsFiles, outputDir, mainModuleName, ignored, testPackage)
    ...
    companion object {
        ...
        private val engineForMinifier = ScriptEngineNashorn()
    }
}
